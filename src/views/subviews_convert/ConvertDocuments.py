...
from src.repository.convert_file_to_pdf import execute_document_to_pdf
...
class ConvertDocuments(ModalsView):
    ...
    def get_input_pdf(self):
        new_file_paths = filedialog.askopenfilenames(defaultextension=".docx", filetypes=[("Document files", "*.docx;")])
    ...
    def find_save_location_and_execute(self):
        ...
        if answer == False:
            return False
        file_path = filedialog.askdirectory() #asksaveasfilename defaultextension=".pdf", filetypes=[("PDF files", "*.pdf")
        
        if file_path:
            self.entry_output_path_pdf.delete(0, tk.END)
            self.entry_output_path_pdf.insert(0, file_path)
            execute_document_to_pdf(files=self.file_paths, path_output_file=self.entry_output_path_pdf.get())
            self.clear_fields()
   ...
   # Método para mover um arquivo para cima na lista
    ...
    def page_convert_documents(self, page):
       # CARREGAR PDF
        ...
        ToolTip(button_get_input_pdf, "Abrir Arquivos de imagem")
        ...
        scrollbar.place(relx=0.92, rely=0.7, anchor=tk.W)#pack(side=tk.RIGHT, fill=tk.Y)
        self.file_paths_listbox.config(yscrollcommand=scrollbar.set)
        scrollbar.config(command=self.file_paths_listbox.yview)

        label_info = tk.Label(page, text="OBS: apenas documentos DOCX e é necessário ter o OFFICE instalado", font=('Arial', 8, 'italic'))
        label_info.place(relx=0.5, rely=0.95, anchor=tk.CENTER)
