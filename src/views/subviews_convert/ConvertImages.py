...
from src.repository.convert_file_to_pdf import execute_imagens_para_pdf
...
class ConvertImages(ModalsView):
    ...
    def get_input_pdf(self):
        new_file_paths = filedialog.askopenfilenames(defaultextension=".png .jpg .jpeg .gif .bmp", filetypes=[("Image files", "*.png;*.jpg;*.jpeg;*.gif;*.bmp")])
        if new_file_paths:
    ...
    def find_save_location_and_execute(self):
        ...
        if file_path:
            ...
            execute_imagens_para_pdf(files=self.file_paths, path_output_file=self.entry_output_path_pdf.get())
            self.clear_fields()
   ...
   # Método para mover um arquivo para cima na lista
    ...
    def page_convert_images(self, page):
       # CARREGAR PDF
        ...
        label_info = tk.Label(page, text="OBS: converte juntando no final")
