...
from src.repository.split_pdf import execute_split_remove_pages_async
import tkinter.ttk as ttk
from src.views.Images import Icons
from tktooltip  import ToolTip
from src.repository.checker_parameters import decode_input_with_str_to_list_int


class SplitRemovePages(ModalsView):
    ...
    def find_save_location_and_execute(self):
        ...
        if save_path:
            ...
            list_pages = decode_input_with_str_to_list_int(input_str=self.entry_bloco_divisao.get())
            if list_pages != False:
                execute_split_remove_pages_async(
                                        pdf_file_origin=self.entry_origin_path_pdf.get(),
                                        path_output_file=self.entry_output_path_pdf.get(),  
                                        list_pages_to_remove=list_pages)
                self.clear_fields()
            else:
                return False
    ...
    def page_split_remove_pages(self, page):
        # CARREGAR PDF
        ...
        label_info_2 = tk.Label(page, text="Cria um arquivo único sem as páginas ou range de páginas indicadas", width=50, font=("Arial", 7, "italic"))
